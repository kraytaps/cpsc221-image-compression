...
private:
   ...
   */
   ...
   // double getSubEntropy(PNG * im, stats * stat_im, pair<int, int> ul, pair<int, int> lr,
   																	// pair<int, int> ctr_ul, pair<int, int> ctr_lr);
   double getEntropy(PNG * bigim, PNG * im, stats * stat_im, int x, int y);

   // PNG * makeChildPNG(PNG * bigim, PNG* im, pair<int, int> ul, pair<int, int> lr);

   PNG * buildBigPNG(PNG * im);

   int sizeHelper(Node* croot);

   HSLAPixel renderHelper(Node* node, int x, int y);
   pair<int, int> NW(pair<int, int> loc, Node* subRoot);
   pair<int, int> NE(pair<int, int> loc, Node* subRoot);
   pair<int, int> SW(pair<int, int> loc, Node* subRoot);
   pair<int, int> SE(pair<int, int> loc, Node* subRoot);
   void clearSubTrees(Node * root);
   void pruneHelper(Node* node, double tol);
   bool isWithinTol(Node* node, double tol, HSLAPixel avg);
